---
title: "What you can do with Media Studio"
description: "Every Picsart Media Studio tool and what it is for — getting files in, building from templates, styling, previewing, rendering, and working with video you already have."
---
...
| Tool | What it does |
|---|---|
| `picsart_media_patch_scene` | Changes one part of a piece without rebuilding the whole thing. |
| `picsart_media_apply_effect` | Adds an effect to one part — a blur, a drop shadow, an outline. |
| `picsart_media_apply_look` | Applies a whole colour treatment, like a vintage black-and-white or a light leak. |
| `picsart_media_apply_motion_preset` | Adds movement — a slow drift across a photo, a pulse, a pop as something appears. |
| `picsart_media_apply_text_animation` | Animates text — typed out letter by letter, faded in, sliding up line by line. |
| `picsart_media_resolve_looks` | Bakes an applied look permanently into the piece. |

## Checking before you render

All three are free, and worth doing.
...
| Tool | What it does |
|---|---|
| `picsart_media_contact_sheet` | Renders a handful of small preview pictures from different moments, so you can see how a piece is looking before committing to the full render. |
| `picsart_media_export` | Renders the finished piece to a real file — a video or a still image, in a range of formats. A single result is saved to your Picsart Drive automatically. |
| `picsart_media_overview` | Lays every slide of a slideshow out as one grid of thumbnails, so you can take in the whole thing at once. |
...
| Tool | What it does |
|---|---|
| `picsart_media_transcribe` | Turns the speech in a video or audio file into text, timed so it can go straight on screen as captions. It can label different speakers, and mark natural places to cut. |
| `picsart_media_describe_video` | Describes what is happening on screen — who is in it, the setting, any text shown, the mood and the colours — and can answer a specific question about it. |
| `picsart_media_reframe_video` | Reframes a video for a different shape of screen, following the subject with a pan and zoom instead of a fixed centre crop. |

## Your Picsart Drive

**Picsart Drive** is the file storage that comes with your Picsart account.
